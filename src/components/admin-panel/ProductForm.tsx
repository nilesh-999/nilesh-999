"use client"
import { useAppDispacth } from '@/redux/hooks';
...
const ProductForm = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    axios.post("/api/add_product", payload).then(res =>{makeToast("Product added Successfully")
      setPayload({
        imgSrc: null,
    fileKey: null,
    name: "",
  ...
  return <form className="flex flex-col gap-4" onSubmit={handleSubmit}><Image className="max-h-[300px] w-auto object-contain rounded-md" src={payload.imgSrc ? payload.imgSrc : "/placeholder.jpg" }width={800} height={500} alt="product_image" />
  <UploadButton
...
<div>
  ...
  <input className='bg-gray-300 w-full px-4 py-2 border outline-pink rounded-md' type="text" value={payload.category} onChange={(e) => setPayload({...payload, category: e.target.value})} 
  required
  />
</div>
<div>
